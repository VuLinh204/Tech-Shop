...
import { addToCart, getUser } from '../../api/Api';
...
const ProductDetails = () => {
    ...
    const validateComment = (comment) => {
        ...
        const isValid =
            trimmedComment.length > 0 && trimmedComment.length <= 255 && /^[\p{L}\p{N}\p{Zs}]+$/u.test(trimmedComment); // Chỉ cho phép ký tự chữ cái, số và khoảng trắng thông thường
    ...
    const handleReply = async (e, parentId, userName) => {
        ...
        const response = await fetch('http://localhost/tech-shop/backend/api/FeedbackApi.php', {
            method: 'POST',
    ...
    const handleDeleteFeedback = async (feedbackId) => {
        ...
        try {
            const response = await axios.delete('http://localhost/tech-shop/backend/api/FeedbackApi.php', {
                data: { id: feedbackId },
            });

            if (response.data.success) {
                setFeedbackList(feedbackList.filter((comment) => comment.id !== feedbackId));
    ...
    const fetchColors = async () => {
        try {
            ...
            const response = await axios.get(`http://localhost/tech-shop/backend/api/ColorApi.php?product_id=${id}`);
            // Kiểm tra nếu response.data có đúng định dạng và chứa mảng
            if (Array.isArray(response.data.data)) {
                // Nếu có, lấy các màu sắc từ mảng data trả về
                const colorData = response.data.data.map((item) => item.name || item.color || item);
    ...
    const fetchFeedback = async () => {
        try {
            const response = await axios.get(`http://localhost/tech-shop/backend/api/FeedbackApi.php?product_id=${id}`);
    ...
    //     // Giả sử bạn có một route thanh toán trên frontend
    //     // Bạn có thể sử dụng React Router để chuyển hướng
    //     window.location.href = '/checkout'; // Hoặc sử dụng useHistory để điều hướng nếu dùng React Router
    // };
    ...
    const handleAddToCart = async (e) => {
        ...
        // Kiểm tra nếu người dùng chưa chọn màu
        if (!selectedColor) {
            alert('Vui lòng chọn màu sản phẩm.');
            return;
        }

        const response = await addToCart(userId, productId, quantityToAdd, selectedColor);

        if (response.status === 'success') {
            setCart((prevCart) => [...prevCart, response.cartItem]);
            navigate('/cart');
        } else {
                                ...
                                <div className="product-row">
                                    ...
                                    <div className="product-details">
                                        ...
                                        <div className="product-actions">
                                            <form className="product-actions-form" onSubmit={handleAddToCart}>
                                                ...
                                                <button type="submit" className="product-buy-now">
                                                    {/* <button type="submit" className="product-buy-now" onClick={handleBuyNow}> */}
                                                    Mua Ngay
                                                </button>
                                            </form>
                                            <hr />
                                        ...
                                        <div className="bg-light p-2 mb-2">
                                            ...
                                            {feedbackList.length > 0 ? (
                                                feedbackList.map((comment) => (
                                                    <div key={comment.id} className="comment bg-light p-3 rounded mb-3">
                                                            ...
                                                            {[1, 2, 3, 4, 5].map((star) => (
                                                                <i
                                                                    ...
                                                                    className={`fa fa-star ${
                                                                        comment.rating >= star ? '' : 'text-secondary'
                                                                    }`}
                                                                ></i>
                                                            ))}
                                                        </div>
